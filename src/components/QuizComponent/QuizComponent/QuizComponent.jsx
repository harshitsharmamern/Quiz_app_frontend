...
function QuizStartButton() {
  ...
  useEffect(()=>{
    ...
    localStorage.setItem('visitedResultPage',false)
    console.log({visitedResultPage});
  },[])
  const handelStart = () => {
    
    try{
      const starttime = { minutes: 0, seconds: 15 };

      localStorage.setItem('timer', JSON.stringify(starttime)); // Update timer value in local storage
